...
`wire-protocol.md` for the message shapes this guide assumes.
...
4. **Convert the wire response into CodeCompass's own types**, in your
   ...
   `../architecture/core-data-model.md` for why that field is
   deliberately narrow).
...
Independent of language, per `wire-protocol.md`:
...
see `docs/external-adapters.md`'s own table for the current entry
(Phase 60: CodeCompass this commit, protocol `0.1.0`, adapter `0.1.0`,
`protocol_version` `1`).

For step-by-step clone/build/submodule-update instructions as an end
user (not an adapter *implementer*), see `docs/external-adapters.md`
directly — that guide is current and is not being reconstructed by this
proposal (see this set's own `README.md`).
