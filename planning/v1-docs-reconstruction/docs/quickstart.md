...
`examples/toy-project/README.md` for a from-scratch minimal example
with `requests`/`click` if you'd rather start with something smaller).
...
(a real transcript, `examples/README.md`). Answer `y`, or skip the
prompt entirely with `--yes`, or cap spend up front with `--budget
<amount>` (checked *before* any API call — if the estimate exceeds it,
Phase B aborts with a non-zero exit, and everything Phase A already
wrote is unaffected). `--budget 0` is a convenient way to try
CodeCompass without spending anything at all.
...
see the note on `pipdeptree` in this proposal's own `README.md`.
...
- `docs/domain/concepts/` (this project's approved domain corpus) — what
  "vendor," "adapter," "digest," and "context packet" precisely mean, if
  a term above was unfamiliar.
