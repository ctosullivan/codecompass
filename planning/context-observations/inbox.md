...
- **origin:** Phase 55 evidence reconciliation, reading Ledgerkit's own
  `validation/codecompass/findings/CC-LK-001.{yaml,md}` (Stage C Phase 2,
  2026-09-16) — Ledgerkit's first-ever real `codecompass` CLI run against
  its own live repository
- **date:** 2026-09-17
- **codecompass_revision:** `e40d8d1`
- **project:** ledgerkit, real live repository (read by CodeCompass's own
  session only via the committed finding — CodeCompass did not itself
  run anything against Ledgerkit this cycle)
- **edge identity:** `.claude/skills/codecompass/SKILL.md` /
  `.claude/commands/discovery.md`, generated by `skill.py::render_tool_skill`/
  `commands.py::render_discovery_command` against a real, 0-vendor
  project for the first time from a genuine cold start
- **observation type:** EDGE_USEFUL (for confirming the architecture is
  sound, not for the content itself, which both projects agree is
  boilerplate at 0 vendors)
- **edge correctness:** correct — the generated content accurately
  reflects 0 tracked vendors; nothing false or misleading.
- **task usefulness:** none, for this specific 0-vendor state — matches
  every prior "0 vendors → LOW advantage" finding from both projects
  (Phase 45's baseline, Phase 54's own detection-only result). `CC-LK-001`
  independently confirms this from Ledgerkit's own side: "content
  matched CodeCompass's own test-fixture template near-verbatim —
  nothing Ledgerkit-specific."
- **default pathway:** reading `reports.py`/`cli.py` directly — which is
  what Ledgerkit's own agent actually did.
- **advantage:** LOW.
- **wrong or misleading?** no.
- **status:** resolved
- **investigation:** none needed — this is the entry-point architecture
  working exactly as designed for a project with nothing yet to route
  to, not a gap. Recorded because it's the first time this exact
  scenario (a genuine cold-start bootstrap against a real, live,
  0-vendor project) has been directly observed, from either side.
- **resolution:** no action. Feeds
  `planning/phase-55-evidence-reconciliation.md`'s §D/§E directly: the
  lightweight "entry point → task discovery → primary evidence"
  direction (this project's own standing design intent) is confirmed
  appropriate, not broken, by this real instance — no redesign
  warranted.

### OBS-010 — bare `codecompass` mutated a real project's `CLAUDE.md` without being asked, for zero current value

- **origin:** Phase 55 evidence reconciliation, reading Ledgerkit's own
  `validation/codecompass/findings/CC-LK-001-baseline-evidence.md`
  (Stage C Phase 2, 2026-09-16)
- **date:** 2026-09-17
- **codecompass_revision:** `e40d8d1`
- **project:** ledgerkit, real live repository
- **edge identity:** `index.py::update_root_claude_md`'s
  `<!-- codecompass:start/end -->` marker-block insertion into
  Ledgerkit's own real `CLAUDE.md`, triggered by a bare
  `codecompass --budget 0` first run
- **observation type:** EDGE_MISLEADING is too strong (nothing false was
  written) — closer to a disclosed friction point about *when* this
  mechanism should act, not *what* it wrote. Filed as EDGE_USEFUL with a
  negative-advantage note, matching this queue's own precedent for a
  "the mechanism worked as designed but the design choice itself has a
  real cost" finding (cf. Phase 45/46's own 0-vendor findings).
- **edge correctness:** correct in content (the inserted table
  accurately reflects 0 vendors) — but the insertion itself was
  unconditional and unreviewed at write time, per `decisions/0017`'s own
  "zero-question auto-discovery" design.
- **task usefulness:** negative in this instance — Ledgerkit's own
  `context-curator` treated the change as zero-value and consciously
  reverted it (`git checkout -- CLAUDE.md`) rather than keep it, per its
  own plan's explicit instruction not to apply it without review.
- **default pathway:** n/a — this is CodeCompass acting unprompted, not
  a comparison against a manual alternative.
- **advantage:** LOW (negative in this instance — the edit had to be
  actively undone).
- **wrong or misleading?** no — the content was accurate, just
  premature/low-value for a 0-vendor project.
- **status:** recorded
- **investigation:** none needed — this is `decisions/0017`'s existing,
  deliberate design (auto-insert unconditionally, so a project's
  routing table is never stale relative to what `sync` last saw)
  functioning exactly as specified. The new information from this real
  instance is narrower: the design's cost/benefit balance looks weaker
  than the original ADR anticipated specifically for a 0-vendor,
  first-ever bootstrap, where the inserted content carries zero current
  information. Not escalated to `context-gaps/` — this is a single
  instance, and reversing `decisions/0017` on one data point would be
  exactly the kind of premature reaction this project's own learning
  lifecycle exists to prevent.
- **resolution:** no action this cycle. Worth revisiting only if a
  second, independent instance of the same friction (a real project
  consciously reverting the auto-inserted block) recurs — feeds
  `planning/phase-55-evidence-reconciliation.md`'s risk register as a
  minor, single-occurrence data point, not a funded fix.
