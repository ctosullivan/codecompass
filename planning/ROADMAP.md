...
| 54 | Heterogeneous reference-material experiment — pin + expose external hledger manual/source material with provenance (Git-backed resolve/lock/fetch/extract), tested against one genuine Ledgerkit Stage C task (`tag:` query semantics) with a baseline-vs-treatment comparison, using Phase 52's context-observation lifecycle *(retargets this slot from the original "Stage D" sketch, "External executable / behavioural context" — that sketch is not discarded, just left without a claimed number — see `phase-54-heterogeneous-reference-material-experiment.md` §0)* | EXPERIMENTAL | planned (plan awaiting review) | [`phase-54-heterogeneous-reference-material-experiment.md`](phase-54-heterogeneous-reference-material-experiment.md) |
| 55 | Stage D decision — GATE DD: is a generalised technical-dependency/provenance concept necessary for v1, informed by Phase 54's evidence? | EXPERIMENTAL → decision | not started | — |
