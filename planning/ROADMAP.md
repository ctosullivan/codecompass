...
| 60 | Minimal Haskell adapter — the full `EcosystemAdapter` interface (5 methods) for Haskell/Stack. Real environment re-verification found `stack ls dependencies` has no JSON mode; `stack dot`'s real GraphViz DOT output (checked live against the real hledger project) is the actual dependency-tree source instead. `vendors.ecosystem` gains `'haskell'` (`_SCHEMA_VERSION` 7→8, mirroring Phase 54c's own just-completed enum-widening precedent). Two judgment calls flagged for review: hand-roll `package.yaml` parsing vs. a new `PyYAML` dependency; whether the one genuinely open design question (Haskell API-surface/export-list extraction) should be routed through Phase 54c's evidence-backed workflow. Not started; plan awaiting review. Full plan: `planning/phase-60-minimal-haskell-adapter.md` | EXPERIMENTAL | not started | [`planning/phase-60-minimal-haskell-adapter.md`](phase-60-minimal-haskell-adapter.md) |
