...
| 55 | Stage D decision — GATE DD: is a generalised technical-dependency/provenance concept necessary for v1, informed by Phase 54's evidence and the Phase 55 evidence-reconciliation package? | EXPERIMENTAL → decision | not started | — |
...
| 61 | hledger cross-language experiment — track both `hledger-lib` and `hledger` (the sibling package holding the real `balance`/`register`/`accounts`/`stats`/`print` command implementations `depth:` semantics span) as real tracked Haskell vendors in a disposable Ledgerkit scratch copy, via the now-working Phase 60 adapter. Re-runs Phase 54b's own `depth:` behavioural-reconstruction question using real structural information instead of the curated document layer, plus a new cross-language half (does CodeCompass help relate hledger's Haskell behaviour to Ledgerkit's own Python `DepthSpec`/`clip_account_name` as the same concept). Baseline = Phase 54b's own real result (LOW context advantage, PASS WITH GAPS, missed `Stats.hs`). Real, disclosed pre-existing bug found during planning and fixed in-phase: `HaskellAdapter.repository_url()` never set `RepositoryLocation.subdirectory`, so `resolve_and_clone` silently cloned the *whole* `hledger` monorepo into a single package's own `vendor/<name>/src/` — confirmed live. `usage.py`'s Haskell import detection (`decisions/0057`'s own stated "Phase 61 scope") is investigated and **not built** — no real consumer for it in this phase's own task, and the one cross-vendor relationship that matters (`hledger` → `hledger-lib`) is already produced free by the existing `depends_on_edges` mechanism. **Amended 2026-09-19** (methodology strengthened, scope unchanged): baseline/treatment inputs made exactly symmetric (recorded, verified commit hashes; a manual post-sync re-pin closing a real `resolve_and_clone`-has-no-ref-pinning gap; identical tools/task wording); evaluation now scores Part 1 (`depth:` reconstruction), Part 2 (cross-language equivalence recognition) and Overall separately, not one undifferentiated verdict; both agents must log which files they read so `context-evaluator` can compute a real rediscovery comparison (reduced/no change/increased overhead); explicit restated boundary that this phase tests adapter-derived context/dependency structure/vendor-source grounding only, not graph-level Haskell symbol integration (`CG-008` stays Phase 62's). Not started; plan awaiting review. Full plan: `planning/phase-61-hledger-cross-language-experiment.md` | EXPERIMENTAL | not started | [`planning/phase-61-hledger-cross-language-experiment.md`](phase-61-hledger-cross-language-experiment.md) |
| 62–63 | Stage F, remainder — adapter-interface consolidation → lightweight ordinary-project smoke test — **GATE DF** *(retargeted 2026-09-17, `decisions/0056`: Technical Clipper is no longer this stage's target — demoted to an optional Phase 63 smoke-test candidate, not roadmap-driving; Rust/npm adapter maturation similarly demoted to later ecosystem-expansion work; original Technical-Clipper content preserved in `v1-redefinition/roadmap.md`, not deleted)*. Not yet planned — depends on Phase 61's own output. | EXPERIMENTAL | not started | [`v1-redefinition/roadmap.md`](v1-redefinition/roadmap.md) §Stage F |
| 64–70 | Stage G — blank-slate doc reconstruction, architecture/ADR/roadmap reconciliation, final self-dogfood + Ledgerkit + Stage F smoke-test confirmation, independent release audit, milestone closeout, release *(was Stage F, phases 60–67 — renumbered +4; Phase 67's Technical-Clipper reference updated per `decisions/0056`)* | COMMITTED (once F completes) | not started | — |
