...
| 70 | **Release redefined CodeCompass v1** — Stage G's seventh and final phase, COMMITTED (gate G9). Drop `.dev0`; `twine upload` (the first-ever publish); `v1.0.0` tag; `[Unreleased]` → dated section. **Irreversible — requires the actual user's own explicit go-ahead**, identical posture to the old Phase 23 Part B pause. **Go-ahead given 2026-09-24** ("Yes, proceed with the release") — gated now only on how to supply real PyPI credentials in this environment (none configured; asked separately, not assumed). *(was Stage F, phase 67 — renumbered +4)* | COMMITTED (gate G9) | planned | [`planning/phase-70-release-v1.md`](phase-70-release-v1.md) |
