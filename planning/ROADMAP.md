...
| 60 | Minimal Haskell adapter — **amended 2026-09-19**: no longer in-process; now the reference implementation of a genuinely **external adapter**, a separate OS process over a small JSON-Lines protocol on stdio (`decisions/0057`), so CodeCompass core never imports ecosystem-specific implementation code — motivated by a future potentially proprietary COBOL/mainframe adapter suite. A real smoke test confirmed a single-file `stack script` using `aeson` builds and runs here (~3s warm). `stack dot`'s real GraphViz output is the dependency-tree source (`stack ls dependencies` has no JSON mode); `_SCHEMA_VERSION` 7→8. Per direct instruction: `package.yaml` parsed via real `PyYAML`; the Haskell API-surface extraction question is mandatorily routed through Phase 54c's evidence-backed workflow; monorepo package-root resolution (`hledger-lib/` within `hledger`) is an explicit tested requirement. Includes a disclosed non-claim: process separation is architectural, not a GPL legal conclusion — real legal review needed for any future proprietary adapter. Not started; plan awaiting review. Full plan: `planning/phase-60-minimal-haskell-adapter.md` | EXPERIMENTAL | not started | [`planning/phase-60-minimal-haskell-adapter.md`](phase-60-minimal-haskell-adapter.md) |
