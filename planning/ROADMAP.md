...
| — (pending numbering decision) | Populate `doc_artifacts.name` for `spec_doc` rows, closing `CG-004` — doubly-corroborated (Phase 54's own scratch-copy experiment + Ledgerkit's independent live-repo `CC-LK-001` finding) doc-to-doc relation gap; before/after validation against the real Ledgerkit repository using `CC-LK-001`'s own three test files, read-only. **Every integer through 63 is already claimed by a pre-written Stage E (56-59)/Stage F (60-63) sketch** — unlike every prior retarget this session, this phase's content doesn't match any of them, and inserting it would require renumbering four already-detailed stages, not just amending one slot. Human decision needed on how to number it before this row's own `—` is resolved — see `phase-55-evidence-reconciliation.md`'s "Human decision gates" section. | EXPERIMENTAL | planned, pending numbering decision | [`phase-55-evidence-reconciliation.md`](phase-55-evidence-reconciliation.md) §G |
