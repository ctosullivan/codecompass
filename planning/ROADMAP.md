...
| 60 | Minimal Haskell adapter — **amended again 2026-09-19**: the protocol and adapter are now separate, differently-licensed **public repositories** checked out as git submodules (`protocol/codecompass-adapter-protocol/` MIT schemas-only, `adapters/haskell/` GPL-3.0-or-later real Stack project), not merely an external process in this repository (`decisions/0058`, superseding that part of `decisions/0057`/the first amendment; first amendment's single-repo design preserved at the plan's own §B). `codecompass` records only a pinned commit SHA per submodule; commits to each repository stay independent; a version-compatibility matrix (CodeCompass ↔ protocol ↔ adapter versions) is required. First amendment (still valid): reference implementation of a genuinely **external adapter** over a small JSON-Lines protocol on stdio, so CodeCompass core never imports ecosystem-specific implementation code — motivated by a future potentially proprietary COBOL/mainframe adapter suite. A real smoke test confirmed `stack`+`aeson` builds and runs here (~3s warm). `stack dot`'s real GraphViz output is the dependency-tree source (`stack ls dependencies` has no JSON mode); `_SCHEMA_VERSION` 7→8. Per direct instruction: `package.yaml` parsed via real `PyYAML`; the Haskell API-surface extraction question is mandatorily routed through Phase 54c's evidence-backed workflow; monorepo package-root resolution (`hledger-lib/` within `hledger`) is an explicit tested requirement. Disclosed non-claim, restated with more force now real separate licenses exist: repo/process separation is architectural, not a GPL legal conclusion — real legal review needed for any future proprietary adapter. New open item: which git host/account the two new public repos are created under is explicitly not decided here. Not started; plan awaiting review. Full plan: `planning/phase-60-minimal-haskell-adapter.md` | EXPERIMENTAL | not started | [`planning/phase-60-minimal-haskell-adapter.md`](phase-60-minimal-haskell-adapter.md) |
| 61–63 | Stage F, remainder — hledger cross-language experiment → adapter-interface consolidation → lightweight ordinary-project smoke test — **GATE DF** *(retargeted 2026-09-17, `decisions/0056`: Technical Clipper is no longer this stage's target — demoted to an optional Phase 63 smoke-test candidate, not roadmap-driving; Rust/npm adapter maturation similarly demoted to later ecosystem-expansion work; original Technical-Clipper content preserved in `v1-redefinition/roadmap.md`, not deleted)*. Not yet planned — depends on Phase 60's own output. | EXPERIMENTAL | not started | [`v1-redefinition/roadmap.md`](v1-redefinition/roadmap.md) §Stage F |
