...
| 53 | Legacy feature rationalisation — full feature inventory (CORE/AGENT/HOST-OUTPUT ADAPTER classification), redundancy map, and per-feature KEEP/REMOVE/DEFER decisions across direct-API enrichment vs. Phase 52's agent-driven path, chat, Skills, and dead code (`rewrite_vendor_toml`) *(retargets this slot from the original "Stage D" sketch, same findings-drive-scope precedent as Phases 49/52 — orthogonal to, does not resolve, the open Stage D-vs-F/G decision)* | EXPERIMENTAL | planned (plan awaiting review gate) | [`phase-53-legacy-feature-rationalisation-plan.md`](phase-53-legacy-feature-rationalisation-plan.md) |
| 54–55 | Stage D — deeper Ledgerkit dogfooding: continued Core development, heterogeneous docs / executable / behavioural context, refine the blueprint — **GATE DD** | EXPERIMENTAL | not started | — |
